...
// Do stuff depending on the event
else {
	// run the code that inolves with this eventTrigger
	if (firstEvent) {
		// Check if the player goes into the male bathroom
		if (objGame.location == 4 and currentLocation = 5) {
			with (objPlayer) {
				canMove = false;
			}
			path = path_add();
			if (mp_grid_path(global.grid, path, x, y, 1194, 2860, false)) {
				path_start(path, spd, path_action_stop, false);
				
				// Once she gets to the door 
				if (floor(x) == 1194 and floor(y) == 2860) {
					if (!audio_is_playing(sndDoor)) audio_play_sound(sndDoor, 5, false);
					currentLocation = 3;
					x = 4399;
					y = 3850;
		...
		// Check if the player goes into the female bathroom
		else if (objGame.location == 5 and currentLocation == 5) {
			// Chase the player
			freeRoam = false;
			sawPlayerHide = true;
		}
		...
		if (alarm[5] == -1) {
			alarm[5] = 30 * room_speed;
		}
		
		// If the player is closeby then attack them
